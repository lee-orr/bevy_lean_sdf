    ...
    S: SDF,
{
    fn value_at_point(&self, point: &Vec3) -> f32 {
        let point = *point;
        match self {
            SdfOperation::Transform(sdf, translation, rotation) => {
                transform_sdf(point, sdf, translation, rotation)
            }
...
impl<L, R> SDF for SdfOperators<L, R>
where
    L: SDF,
    R: SDF,
{
...
fn transform_sdf<S>(point: Vec3, sdf: &S, translation: &Vec3, rotation: &Quat) -> f32
where
    S: SDF,
{
    let transform =
...
fn scale_sdf<S>(point: Vec3, sdf: &S, scale: f32) -> f32
where
    S: SDF,
{
...
fn union<L, R>(point: Vec3, left: &L, right: &R) -> f32
where
    L: SDF,
    R: SDF,
{
    left.value_at_point(&point)
        .min(right.value_at_point(&point))
}

fn subtraction<L, R>(point: Vec3, left: &L, right: &R) -> f32
where
    L: SDF,
    R: SDF,
{
...
fn intersection<L, R>(point: Vec3, left: &L, right: &R) -> f32
where
    L: SDF,
    R: SDF,
{
    left.value_at_point(&point)
        .max(right.value_at_point(&point))
}
...
mod tests {
    ...
    #[test]
    fn rotates_a_sdf() {
        let sdf = SdfOperation::Transform(
    ...
    fn union_of_sdfs() {
        ...
        let sdf_b =
            SdfOperation::Transform(SdfPrimitive::Sphere(1.), -1. * Vec3::X, Quat::IDENTITY);
